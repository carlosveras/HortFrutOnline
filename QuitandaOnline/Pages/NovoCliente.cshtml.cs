...
namespace HortFrutOnline.Pages
{
    public class NovoClienteModel : PageModel
    {
        ...
        private HortFrutOnlineContext _context;
        private readonly IEmailSender _emailSender;
        private UserManager<AppUser> _userManager;
        private RoleManager<IdentityRole> _roleManager;

        public NovoClienteModel(HortFrutOnlineContext context, IEmailSender emailSender,
			...
			if (await TryUpdateModelAsync(cliente, Cliente.GetType(), nameof(Cliente)))
			{
				...
				if (result.Succeeded)
				{
					...
					if (afetados > 0)
					{
						...
						msg.Append("<h1>Horti Frut Online :: Confirmação de E-mail</h1>");
						msg.Append($"<p>Por favor, confirme seu e-mail " +
							$"<a href='{HtmlEncoder.Default.Encode(urlConfirmacaoEmail)}'>clicando aqui</a>.</p>");
						msg.Append("<p>Atenciosamente,<br>Equipe de Suporte Horti Frut Online</p>");
